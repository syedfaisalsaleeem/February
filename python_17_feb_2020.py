...
##            self.entry.setCursorPosition(2)
    ...
    def shift1(self):
        ...
        self.destroy()
        gc.collect()
        self.capital_keyboard1=capital_keyboard(self.entry.text())
        self.capital_keyboard1.show()
        
        
            
class capital_keyboard(QWidget):
    def __init__(self,entry_small):
        super().__init__()
        self.title = "App"
        self.top = 0
        self.left = 100
...
##            self.entry.setCursorPosition(2)
    ...
    def shift1(self):
        ...
        self.destroy()
        gc.collect()
        self.keyboard1=keyboard(self.entry.text())
        self.keyboard1.show()
...
class adduser(QWidget):
    ...
    def InitUI(self):
        ...
        self.e_employer = QLineEdit(self)
        ...
        self.e_designation = QLineEdit(self)
...
##        gc.collect()
    ...
    def call_edit(self):
        ...
        try:
            ...
            conn = sqlite3.connect('faisal.db')
            c = conn.cursor()
            c.execute(f"DELETE from Userdata where ActionKey={self.editdata}") 
            c.execute("INSERT INTO Userdata(Name,Employerid,Contact,Designation,ActionKey) VALUES(?,?,?,?,?)",(self.user_name.strip(),self.user_employerid.strip(),self.user_contact.strip(),self.user_designation.strip(),self.editdata))
            conn.commit()
            c.close()
            conn.close()
            self.close()
